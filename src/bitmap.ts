...
export function padBitmap(
  ...
  const newWidth = bitmap.width + paddingLeft + paddingRight;
  const newHeight = bitmap.height + paddingTop + paddingBottom;

  // Create arrays filled with padding color for top/bottom
  const filled = Array((paddingTop + paddingBottom) * newWidth).fill(color);

  // Create padding arrays for left/right
  const left = Array(paddingLeft).fill(color);
  const right = Array(paddingRight).fill(color);

  // Convert bitmap data to 2D array of rows
  const rows: number[][] = [];
  for (let y = 0; y < bitmap.height; y++) {
    const row = bitmap.data.slice(y * bitmap.width, (y + 1) * bitmap.width);
    rows.push(row);
  }

  // Build new data array with padding
  const topPadding = filled.slice(0, paddingTop * newWidth);
  const bottomPadding = filled.slice(filled.length - paddingBottom * newWidth);

  const middleRows = rows.reduce((acc, row) => {
  ...
  return {
    width: newWidth,
    height: newHeight,
    data: [...topPadding, ...middleRows, ...bottomPadding],
